...
import {
  searchBoxOpenValue,
  setSearchBoxOpen,
  useSearchBoxOpenDispatch,
  useSearchBoxOpenSliceSelector,
} from "../../redux/searchBoxOpenSlice";

interface SearchPropsTypes {
  showSearchButton?: boolean;
}

export const Search = ({ showSearchButton = true }: SearchPropsTypes) => {
  const path_name = usePathname();
  const searchMetaInfo = useSliceSelector(searchMetaInfoValues);
  const dispatch = useSliceDispatch();
  const isSearchBoxOpen = useSearchBoxOpenSliceSelector(searchBoxOpenValue);
  const searchboxOpenDispatch = useSearchBoxOpenDispatch();
  ...
  useEffect(() => {
    const down = (e: KeyboardEvent) => {
      if (e.key === "k" && (e.metaKey || e.ctrlKey)) {
        ...
        searchboxOpenDispatch(setSearchBoxOpen(!isSearchBoxOpen));
      }
    };
  ...
  useEffect(() => {
    if (!setSearchBoxOpen) return;
    searchboxOpenDispatch(setSearchBoxOpen(false));
  }, [path_name]);
    ...
    <>
      {showSearchButton && (
        <PMButton
          ...
          variant="secondary"
          radius="tablet"
          className={`px-2 py-1 flex justify-center items-center gap-1 group fixed top-5 right-5 z-30 searchTutorialRoot`}
        >
          <SearchIcon
              ...
              <NoResultsBoundary fallback={<NoResults />}>
                ...
                <div className={`h-full`}>
                  ...
                  >
                    ...
                    >
                      <div
                        onClick={handleOpenSearchMobInfo}
