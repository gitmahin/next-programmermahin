...
export const QuickLearnPagination = () => {
  ...
  return (
    <div className="flex justify-between items-center gap-10 my-16">
      {prev && (
        <Link href={prev?.path || ""}>
          <PMButton variant="secondary" className="py-1 px-5 rounded group flex justify-center items-center gap-1">
            <ChevronLeft size={LUCIDE_DEFAULT_ICON_SIZE} className="text-text-svg_default_color group-hover:text-text-color_1" />
      ...
      {next && (
        <Link href={next?.path || ""}>
          <PMButton variant="secondary" className="py-1 px-5 rounded group flex justify-center items-center gap-1">
            ...
            <ChevronRight size={LUCIDE_DEFAULT_ICON_SIZE} className="text-text-svg_default_color group-hover:text-text-color_1" />
            </PMButton>
        </Link>
      )}
    </div>
